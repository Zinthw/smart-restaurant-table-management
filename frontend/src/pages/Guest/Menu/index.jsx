...
export default function GuestMenu() {
  ...
  useEffect(() => {
    const initData = async () => {
      ...
      try {
        ...
        setModifierGroups([
          { id: 1, name: 'Kích cỡ (Size)', selection_type: 'single', options: [{ name: 'M', price: 0 }, { name: 'L', price: 10000 }] },
          { id: 3, name: 'Topping', selection_type: 'multi', options: [{ name: 'Trân châu đen', price: 5000 }, { name: 'Thạch trái cây', price: 5000 }] }
        ]);

        setItems([
          {
            id: 1, name: 'Bò Bít Tết Sốt Tiêu', price: 250000, category_id: 2, status: 'active', order_count: 150,
            is_recommended: true,
            description: 'Thịt bò nhập khẩu mềm ngon, sốt tiêu đen đậm đà, kèm khoai tây và salad.',
            photos: [{ id: 1, url: 'https://images.unsplash.com/photo-1600891964092-4316c288032e?w=800', is_primary: true }],
            modifier_group_ids: []
          },
          {
            id: 3, name: 'Trà Sữa Trân Châu', price: 45000, category_id: 3, status: 'active', order_count: 500,
            is_recommended: true,
            description: 'Trà sữa truyền thống nấu từ trà lá đậm vị, kết hợp sữa béo ngậy.',
            photos: [{ id: 3, url: 'https://images.unsplash.com/photo-1556679343-c7306c1976bc?w=800', is_primary: true }],
            modifier_group_ids: [1, 3]
          },
          {
            id: 4, name: 'Bánh Flan Caramel', price: 25000, category_id: 4, status: 'sold_out', order_count: 230,
            is_recommended: false,
            description: 'Bánh flan mềm mịn, sốt caramel đắng nhẹ đúng điệu kiểu Pháp.',
            photos: [{ id: 4, url: 'https://images.unsplash.com/photo-1551024506-0bccd828d307?w=800', is_primary: true }],
            modifier_group_ids: []
          },
        ]);
      } finally {
  ...
  const handleOptionChange = (groupId, option, selectionType) => {
    ...
    if (selectionType === 'single') {
      ...
      if (isSelected) {
        currentOptions[groupId] = existing.filter(item => item.name !== option.name);
      } else {
  ...
  const getItemModifiers = (item) => {
    ...
    return modifierGroups.filter(group => item.modifier_group_ids.includes(group.id));
  };
  ...
  return (
    <div className="mobile-container">
        ...
        {isAdmin && (
          <button 
            ...
            style={{
              ...
              border: 'none', borderRadius: '15px', padding: '4px 10px',
        ...
        <div style={{ textAlign: 'right', marginBottom: '5px' }}>
            <button 
                ...
                style={{
                    ...
                    borderRadius: '8px', padding: '5px 12px', fontSize: '12px', fontWeight: 'bold'
                }}
            >
                Đăng nhập tích điểm
            </button>
              ...
              <div className="menu-item-image">
                {item.is_recommended && (
                  <span style={{
                    ...
                    borderBottomRightRadius: 8, fontWeight: 'bold', zIndex: 1
                  }}>
                    ★ Hot
                  </span>
                )}
              ...
              <div className="menu-item-info">
                <div>
                  ...
                  {item.status === 'sold_out' && (
                    <span className="menu-item-status sold-out">Hết hàng hôm nay</span>
                  )}
                </div>
      ...
      {selectedItem && (
        <div className="item-detail-overlay" onClick={() => setSelectedItem(null)}>
          <div className="item-detail-sheet" onClick={e => e.stopPropagation()}>
            <button className="close-sheet" onClick={() => setSelectedItem(null)}>&times;</button>

            <div className="detail-image" style={{ position: 'relative' }}>
              {getPrimaryImage(selectedItem) ? (
                <img src={getPrimaryImage(selectedItem)} alt="" />
              ) : (
                <div style={{ fontSize: 50, display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100%' }}>🍽️</div>
              )}
            </div>
            ...
            <div className="detail-body">
              <div className="detail-header">
                ...
                <span className="detail-price">{selectedItem.price.toLocaleString()}đ</span>
              </div>
                  ...
                  <div key={group.id} className="modifier-group">
                    <div className="mod-group-header">
                      ...
                      <span className="mod-group-type">
                        {group.selection_type === 'single' ? 'Chọn 1' : 'Được chọn nhiều'}
                      </span>
                    </div>
                      ...
                      {group.options.map((opt, idx) => (
                        <label key={idx} className="mod-option-item">
                          <input
                            ...
                            name={`group-${group.id}`}
                            checked={(selectedOptions[group.id] || []).some(item => item.name === opt.name)}
                            onChange={() => handleOptionChange(group.id, opt, group.selection_type)}
                          />
            ...
            <div className="detail-footer">
              ...
              >
                {selectedItem.status === 'sold_out' ? 'Tạm hết hàng' : `Thêm vào giỏ - ${calculateTotalPrice().toLocaleString()}đ`}
              </button>
            </div>
          </div>
        </div>
      ...
      <div className="bottom-nav">
        ...
        <div className="nav-item" style={{ position: 'relative' }} onClick={() => navigate('/customer/login')}>
          <span className="nav-icon">&#128722;</span>
