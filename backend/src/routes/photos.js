...
router.post('/:id/photos', upload.array('photos', 5), async (req, res, next) => {
  try {
    ...
    const itemCheck = await db.query("SELECT 1 FROM menu_items WHERE id = $1", [itemId]);
    ...
    res.status(201).json({ message: 'Upload successful', data: uploadedPhotos });
  } catch (err) { next(err); }
});
...
router.delete('/:id/photos/:photoId', async (req, res, next) => {
  try {
    ...
    if (rows.length === 0) return res.status(404).json({ message: 'Photo not found or does not belong to this item' });
...
router.patch('/:id/photos/:photoId/primary', async (req, res, next) => {
  ...
  try {
    ...
    await client.query("UPDATE menu_item_photos SET is_primary = false WHERE menu_item_id = $1", [id]);

    await client.query("UPDATE menu_item_photos SET is_primary = true WHERE id = $1", [photoId]);
