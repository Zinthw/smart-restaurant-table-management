...
router.post('/modifier-groups', async (req, res, next) => {
  try {
    const { name, selection_type, min_selection, max_selection, required } = req.body;
    
    if (!name) return res.status(400).json({ message: 'Group name is required' });
    if (selection_type === 'multiple') {
       if (min_selection > max_selection) {
           return res.status(400).json({ message: 'Min selection cannot be greater than Max selection' });
       }
    }
    ...
    const { rows } = await db.query(
      ...
      [name, selection_type || 'single', min_selection || 0, max_selection || 1, required || false]
    );
...
router.put('/modifier-groups/:id', async (req, res, next) => {
  try {
    ...
    const { rows } = await db.query(
      ...
      [name, selection_type, min_selection || 0, max_selection || 1, required || false, req.params.id]
    );
...
router.get('/modifier-groups', async (req, res, next) => {
  try {
    const { rows } = await db.query("SELECT * FROM modifier_groups ORDER BY created_at DESC");
    res.json(rows);
  } catch (err) { next(err); }
});
...
router.post('/modifier-groups/:id/options', async (req, res, next) => {
  try {
    ...
    if (price_adjustment < 0) return res.status(400).json({ message: 'Price cannot be negative' });
...
router.get('/modifier-groups/:id/options', async (req, res, next) => {
  try {
    const { rows } = await db.query("SELECT * FROM modifier_options WHERE group_id = $1", [req.params.id]);
    res.json(rows);
  } catch (err) { next(err); }
});
...
router.post('/items/:id/modifier-groups', async (req, res, next) => {
  ...
  try {
    ...
    await client.query("DELETE FROM menu_item_modifier_groups WHERE menu_item_id = $1", [itemId]);
