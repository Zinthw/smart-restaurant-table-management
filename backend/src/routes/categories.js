...
router.post("/", async (req, res, next) => {
  try {
    ...
    if (name.length > 100)
      return res
        .status(400)
        .json({ message: "Category name is too long (max 100 characters)" });
    if (sort_order !== undefined && sort_order < 0)
      return res.status(400).json({ message: "Sort order must be >= 0" });

    const dup = await db.query(
      "SELECT 1 FROM menu_categories WHERE name = $1 AND deleted_at IS NULL",
      [name]
    );
    if (dup.rowCount > 0)
